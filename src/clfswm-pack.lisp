...
(labels ((redisplay (frame window)
         ...
         (readjust-all-frames-fl-size (parent)
           (dolist (child (frame-child parent))
             (when (frame-p child)
               (setf (frame-x child)
                     (x-px->fl (x-drawable-x (frame-window child)) parent)
                     (frame-y child)
                     (y-px->fl (x-drawable-y (frame-window child)) parent)
                     (frame-w child)
                     (w-px->fl (anti-adj-border-wh
                                (x-drawable-width (frame-window child)) child) parent)
                     (frame-h child)
                     (h-px->fl (anti-adj-border-wh
                                (x-drawable-height (frame-window child)) child) parent))))))
          ...
          (resize-window window orig-x orig-y
                         (lambda ()
                           (let ((resize-w t)
                             ...
                             (setf (frame-w frame)
                                   (w-px->fl
                                    (anti-adj-border-wh (x-drawable-width window) frame) parent)
                                   (frame-h frame)
                                   (h-px->fl
                                    (anti-adj-border-wh (x-drawable-height window) frame) parent))
                             (multiple-value-bind (x y) (xlib:query-pointer *root*)
                               (when (> x lx)
                                 (let ((x-found (find-edge-right frame parent)))
                                   (when (< (abs (- x-found (frame-x2 frame))) snap-size)
